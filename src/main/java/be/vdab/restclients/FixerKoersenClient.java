...
public class FixerKoersenClient implements KoersenClient {
	private final static Logger LOGGER = Logger.getLogger(FixerKoersenClient.class.getName());
			 private final URI fixerURL;
			 private final RestTemplate restTemplate;
	
	FixerKoersenClient(@Value("${fixerKoersenURL}") URI fixerURL, RestTemplate restTemplate) {
	...
	public BigDecimal getDollarKoers() {
		try {
			USDRates rate = restTemplate.getForObject(fixerURL, USDRates.class);
